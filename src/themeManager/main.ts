...
interface ThemeManifest {
    ...
    source?: string;
    updateSrc?: string;
    supportsZelcordTitlebar?: boolean;
}
    ...
    if (instance > 1) {
        ...
        ipcMain.on("installBDTheme", async (_event, link: string) => {
            try {
                ...
                if (code.includes(".titlebar")) manifest.supportsZelcordTitlebar = true;
                else manifest.supportsZelcordTitlebar = false;
